...
| 17 | User uploads custom dataset but bundled outputs silently flow through | High | High | Resolved in v0.2: explicit Demo Mode vs Custom Eval Mode toggle blocks the mix. Custom Eval Mode requires the user to upload the dataset **and** both output files; bundled paths are not exposed. |
| 18 | Rubric is hard-coded for customer support; non-support data scores ~1/5 | High | High | Resolved in v0.2: `scoring_method` defaults to generic `rubric` (semantic overlap), with `rubric_support` opt-in. UI shows a banner when both averages drop below 1.6/5. |
| 19 | Users believe editing a prompt re-runs the eval | High | High | Resolved in v0.2: Variants section caption and per-mode helper text state explicitly that prompts are metadata. Model vs Model relabelled to "Output Source A/B" to reinforce. Demo Mode adds an inline outputs editor so iteration is possible without leaving the page. |
| 20 | Hugging Face Spaces deploy needs Streamlit version pinning | Low | Low | README YAML frontmatter pins `sdk_version: 1.30.0`; the Space starts `app.py` automatically. CPU Basic tier sufficient (no model calls, no GPU). |
